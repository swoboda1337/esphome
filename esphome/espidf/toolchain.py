...
def has_outdated_files():
    ...
    Returns True if required build files are missing or if our resolved
    sdkconfig (``sdkconfig.<name>.esphomeinternal``) is newer than
    CMakeCache.txt. The sdkconfig snapshot is the canonical "what state
    did ESPHome resolve the YAML to" signal -- any change in build flags,
    enabled components, framework version, or target ends up rewriting
    its content (we embed a ``# ESPHOME_IDF_VERSION=`` comment line for
    the version case where the option set would otherwise be identical).
    ...
    - ``$IDF_PATH`` and CMake's ``build/config/`` -- both have mtime
      ...
      hash subsumes the meaningful signal.
    """
    ...
    return (
        sdkconfig_internal_path.exists()
        and os.path.getmtime(sdkconfig_internal_path) > cmakecache_txt_mtime
    )
