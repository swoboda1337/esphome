...
Invoked as ``python espidf_runner.py <script_path> [script args...]``.
...
def main() -> int:
    # ---- sys.path fix-up ---------------------------------------------------
    #
    # When Python runs this file as ``python espidf_runner.py``, it prepends
    # the script's directory — ``<site-packages>/esphome/`` — to
    # ``sys.path[0]``. That directory contains esphome-internal modules whose
    # names collide with stdlib modules (notably ``types.py``), so *any*
    # subsequent import that transitively touches ``types`` (``runpy``,
    # ``pathlib``, ``functools``, ``typing``, ...) resolves the wrong module
    # and fails with:
    #
    #     ImportError: cannot import name 'GenericAlias' from 'types'
    #
    # Drop that entry before importing anything else. ``sys`` is a built-in
    # so importing it at module level earlier did not trigger the shadow.
    ...
    if len(sys.argv) < 2:
        print(
            "usage: espidf_runner.py <script_path> [args...]",
            file=sys.stderr,
        )
        return 2
