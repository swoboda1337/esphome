...
async def new_fastled_light(config):
    ...
    # Pinned to the 3.10.3 tag SHA because the PlatformIO registry is stuck on 3.9.16.
    # Post-3.10.3 commits that would add IDF 6 support currently have multiple internal
    # build regressions (audio namespace, SIMD on esp8266, etc.) so hold here until upstream
    # cuts a cleaner release.
    cg.add_library(
        "FastLED",
        "3.10.3",
